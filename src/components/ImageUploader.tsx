...
export const ImageUploader: React.FC<ImageUploaderProps> = ({
  ...
  const validateFile = (file: File): string | null => {
    if (!acceptedTypes.includes(file.type)) {
      return tibetan.errors.format;
    }
    if (file.size > maxSize) {
      return tibetan.errors.size;
    }
    return null;
  };
  ...
  return (
    <div className="space-y-6">
      <div className="text-center mb-6">
        <h2 className="text-2xl font-bold text-gray-800 mb-2 leading-[1.5]">{tibetan.uploadTitle}</h2>
        <p className="text-gray-600">{tibetan.uploadSubtitle}</p>
      </div>
      ...
      {!uploadedFile ? (
        ...
        <div className="bg-gradient-to-br from-green-50 to-emerald-50 border border-green-200 rounded-2xl p-6">
          ...
          <div className="bg-white/60 rounded-xl p-4">
            <p className="text-sm text-gray-600 mb-2">{tibetan.fileInfo}:</p>
            <div className="grid grid-cols-2 gap-4 text-sm">
              <div>
                <span className="text-gray-500">{tibetan.fileSize}:</span>
                <span className="ml-2 font-semibold">{formatFileSize(uploadedFile.size)}</span>
              </div>
              <div>
                <span className="text-gray-500">{tibetan.format}:</span>
